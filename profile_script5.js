...
function addstory(){
  ...
  collect.addEventListener('click', addon = (event) => {
    ...
    // for(let a of storyOne){   //dont need this actually
      ...
      // collectbtn.insertBefore(elt('div',{class:'box'},storyOne[counter].text),collectbtn.firstChild)
      ...
      console.log(` waited : ${waited}`);
          target.addEventListener('click',()=>{
            ...
            if(waited){
              ...
              if(boxes){console.log(boxes,'BOXES!!!')}
              ...
              fading();
              function fading(){
              ////  if(collectbtn.firstChild.nextSibling.style){collectbtn.firstChild.nextSibling.style.opacity = opac -0.1;}
                // if(collectbtn.lastChild.style){collectbtn.lastChild.style.opacity = opac -0.1;}
                // if(collectbtn.lastChild.previousSibling.style){collectbtn.lastChild.previousSibling.style.opacity = opac -0.1;}

                ////if(collectbtn.firstChild.nextSibling.nextSibling){console.log(collectbtn.firstChild.nextSibling.nextSibling,'NEXTNEXT sibling')};
                let decrease;
                decrease -= 0.1;
                // for(let k = 0; k < boxes.length-1; k++){
                //   // if(boxes[counter].style){boxes[counter].style.opacity = opac -0.6}
                //   // if(boxes[counter-1].style){boxes[counter-1].style.opacity = opac -0.4}
                //   // if(boxes[counter-2].style){boxes[counter-2].style.opacity = opac -0.3}
                //   // if(boxes[counter-3].style){boxes[counter-3].style.opacity = opac -0.2}
                //   // if(boxes[counter-4].style){boxes[counter-4].style.opacity = opac -0.1}
                // if(boxes[counter-k].style){boxes[counter-k].style.opacity = opac -(k/10)}
                // // vs
                // // if(boxes[counter-k].style){boxes[counter-k].style.opacity = opac}
                // // vs
                // if(boxes[counter-k].style){boxes[counter-k].style.opacity = opac - 0.1}
                //
                // }
                for(let i = 0; i < boxes.length; i++){
                  console.log(boxes.length,'boxes length here');
                  if(boxes[i].style){
                    //
                    // boxes[0].style.opacity = 1;
                    // boxes[1].style.opacity = 0.9;
                    // boxes[2].style.opacity = 0.8;
                    // boxes[3].style.opacity = 0.7;
                    // boxes[4].style.opacity = 0.6;
                    // boxes[5].style.opacity = 0.5;
                    // boxes[6].style.opacity = 0.4;
                    // boxes[7].style.opacity = 0.3;
                    // boxes[8].style.opacity = 0.2;  // this works
                ...
                for(boxclass of boxes){
                  console.log(boxclass,'these are box classes');
                  console.log(boxclass.style.opacity = 1 ? true : false);
                ...
                if(boxes[0].style){console.log('boxes got style')}

                  for (each_one of collectbtn.childNodes){
                    if (waited){
                      if(each_one.style){
                        ...
                        // }
                        // console.log(each_one, 'each_one hereee');
                          // each_one.style.opacity = opac - 0.1;
